...
def CaminoeulerianoCaballo(n,camino,v,limite_Tablero):
    ...
    if n<limite_Tablero:
        ...
        while i<len(nList) and not Terminado:
            if nList[i].getColor()=='blanco':
                Terminado=CaminoeulerianoCaballo(n+1,camino,nList[i],limite_Tablero)
            i=i+1
        if not Terminado:
            camino.pop()
            v.setColor('blanco')
...
def Warnsdorff(n):#Recive el grafo camino del caballo 
    re = [] #creamos la lista de respuesta.
...
def CaminoeulerianoCaballoWarnsdorff(n,camino,v,limite_Tablero):
    v.setColor('Negro') #Para que el algoritmo sepa que ya visitamos un vertice le vamos a poner el color negro
    camino.append(v)
    if n<limite_Tablero:
        nList=Warnsdorff(v)
        ...
        while i<len(nList) and not Terminado:
            if nList[i].getColor()=='blanco':
                Terminado=CaminoeulerianoCaballoWarnsdorff(n+1,camino,nList[i],limite_Tablero)
            i=i+1
        if not Terminado:
            camino.pop()
            v.setColor('blanco')
...
CaminoeulerianoCaballo(0,CaminoHamiltoniano,Pos_Inicial,24)
print("Camino euleriano" )
res=[]
for i in CaminoHamiltoniano:
    res.append(i.getid())
print(res)
end = time.time()
...
CaminoeulerianoCaballoWarnsdorff(0,CaminoHamiltoniano2,Pos_Inicial,24)
res1=[]
print(" Camino euleriano CaballoWarnsdorff" )
for i in CaminoHamiltoniano:
    res1.append(i.getid())
print(res1)
end2=time.time()
    
print("Tiempo Función Camino euleriano Warnsdorff : ", end2 - start2)
